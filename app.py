...
def preparationData():
    ...
    # load main datasets
    apptrainPart1 , apptrainPart2=pd.read_csv("./data/application_trainPart1.csv"),pd.read_csv("./data/application_trainPart2.csv")
    app_train=pd.concat([apptrainPart1,apptrainPart2])
...
def MainAppData(idUser):
    ...
    if currUser.empty == False:
        ...
        lgbm.fit(X_train[:100], y_train[:100])
        finalResProba=lgbm.predict_proba(currUser)[:, 1]
        print("jvais bien a la fin de lgbm")
    else :
        finalResProba=[-1]
        print("jvais bien mais y a pas le user")
    print("finalResProba:",round(finalResProba[0]*5050))
    return render_template('myapires.html',results=round(finalResProba[0]*5050))
